...
const uploadCartItems = async (cart, setCart, VendorName, total, navigation) => {
  ...
  try {
    ...
    const filteredCart = cart.map(({ id, name, price, quantity }) => ({
      ...
      await addDoc(orderRef, { cart: filteredCart, VendorName:VendorName, date:formattedDate, location: "29A, Tilak Nagar", status:"Pending", total:total, orderId: uniqueOrderId });
